...
export function AnimalForm({ animal, onSuccess, onCancel }: AnimalFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const data = {
        ...formData,
        peso_nascimento: formData.peso_nascimento ? Number.parseFloat(formData.peso_nascimento) : null,
      ...
      if (animal?.id) {
        const { error } = await supabase.from("animais").update(data).eq("id", animal.id)
        if (error) throw error
      } else {
        const { error } = await supabase.from("animais").insert({ ...data, status: "ativo" })
        if (error) throw error
      }
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, numero_brinco: e.target.value })}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: OrigemAnimal) => setFormData({ ...formData, origem: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.raca_id} onValueChange={(value) => setFormData({ ...formData, raca_id: value })}>
                <SelectTrigger>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={formData.mae_id} onValueChange={(value) => setFormData({ ...formData, mae_id: value })}>
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.pai_id} onValueChange={(value) => setFormData({ ...formData, pai_id: value })}>
                <SelectTrigger>
            ...
            {formData.genero === "F" && (
              <div className="rounded-lg border p-4 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="vacina_brucelose" className="text-sm font-medium">
                      Vacina de Brucelose
                    </Label>
                    <p className="text-xs text-muted-foreground">Obrigatória para fêmeas de 3 a 8 meses</p>
                  </div>
                  <Switch
                    id="vacina_brucelose"
                    checked={formData.vacina_brucelose}
                    onCheckedChange={(checked) => setFormData({ ...formData, vacina_brucelose: checked })}
                  />
                </div>
                {formData.vacina_brucelose && (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({ ...formData, data_brucelose: e.target.value })}
                    />
                  </div>
                )}
              </div>
