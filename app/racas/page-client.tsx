...
export function RacasPageClient({ initialRacas }: RacasPageClientProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (editingRaca) {
        const { error } = await supabase.from("racas").update(formData).eq("id", editingRaca.id)
              ...
              {racas.length > 0 ? (
                racas.map((raca) => (
                  <TableRow key={raca.id}>
                    ...
                    <TableCell className="text-muted-foreground">{raca.descricao || "-"}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" onClick={() => openDialog(raca)}>
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(raca.id)}>
