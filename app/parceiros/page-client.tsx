...
export function ParceirosPageClient({ initialParceiros }: ParceirosPageClientProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (editingParceiro) {
        const { error } = await supabase.from("parceiros").update(formData).eq("id", editingParceiro.id)
        if (error) throw error
      } else {
        const { error } = await supabase.from("parceiros").insert({ ...formData, ativo: true })
        if (error) throw error
      }
              ...
              {parceiros.length > 0 ? (
                parceiros.map((parceiro) => (
                  <TableRow key={parceiro.id}>
                    ...
                    <TableCell>
                      <Badge variant={tipoBadgeVariant[parceiro.tipo]}>{tipoLabel[parceiro.tipo]}</Badge>
                    </TableCell>
                    <TableCell>{parceiro.documento ? formatDocument(parceiro.documento) : "-"}</TableCell>
                    <TableCell>{parceiro.telefone ? formatPhone(parceiro.telefone) : "-"}</TableCell>
                    <TableCell>{parceiro.email || "-"}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" onClick={() => openDialog(parceiro)}>
          ...
          <div className="space-y-4 py-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={formData.tipo}
                  onValueChange={(v: TipoParceiro) => setFormData({ ...formData, tipo: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
